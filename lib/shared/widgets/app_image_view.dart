...
class AppImageView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (raw.isEmpty) {
      ...
      final fullUrl = raw.startsWith('/') ? '$serverHost$raw' : '$serverHost/$raw';
      imageWidget = _buildNetworkImage(fullUrl);
    } else if (raw.startsWith('/') || raw.contains(':\\') || raw.startsWith('file://')) {
  ...
  Widget _buildBase64Image(String dataUri) {
    try {
      final base64String = dataUri.contains(',') ? dataUri.split(',').last : dataUri;
  ...
  Widget _buildFallbackImage() {
    final fallbackUrl = getFallbackForMaterial(name: materialName, category: categoryName);
    return CachedNetworkImage(
