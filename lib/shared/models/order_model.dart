...
class OrderModel {
  ...
  final double total;
  final String
  orderStatus; // 'PENDING_PAYMENT', 'CONFIRMED', 'OUT_FOR_DELIVERY', 'DELIVERED', 'COMPLETED', 'CANCELLED'
  final String
  ...
  factory OrderModel.fromJson(Map<String, dynamic> json) {
    final rawItems =
        (json['items'] as List?)
            ?.map(
              (e) =>
                  OrderItemModel.fromJson(e is Map<String, dynamic> ? e : {}),
            )
            .toList() ??
        [];
    ...
    if (json['deliveryLocation'] != null &&
        ...
        json['deliveryLocation']['coordinates'] is List) {
      coords = (json['deliveryLocation']['coordinates'] as List)
      ...
      if (dMap['latitude'] != null && dMap['longitude'] != null) {
        coords = [
          (dMap['longitude'] as num).toDouble(),
          (dMap['latitude'] as num).toDouble(),
        ];
      }
    }
    ...
    if (json['deliveryAddress'] is Map) {
      ...
      final street =
          d['street']?.toString() ?? d['streetAddress']?.toString() ?? '';
      ...
      formattedAddress =
          s['street']?.toString() ?? s['city']?.toString() ?? 'Adama';
    ...
    if (phone == null || phone.isEmpty) {
      phone =
          json['contactPhone']?.toString() ?? json['phoneNumber']?.toString();
    }
    ...
    return OrderModel(
      ...
      orderNumber:
          ...
          (json['_id']?.toString().substring(0, 8).toUpperCase() ?? 'ORD'),
      items: rawItems,
      ...
      contactPhone: phone,
      trackingNumber:
          json['trackingNumber']?.toString() ?? json['orderNumber']?.toString(),
